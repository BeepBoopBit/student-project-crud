...
class Database(QMainWindow):
    ...
    def useDatabase(self): 
        ...
        try:
            ...
            Widget.addWidget(MainCrudWindow(self.API));      #3  (Main)
            Widget.addWidget(FilterTable(self.API));         #4  (Filter)
            Widget.addWidget(GroupingTable(self.API));       #5  (Group)
            Widget.addWidget(ModifyTable(self.API));         #6  (Modify)

            # TABLE

            Widget.addWidget(NameTable(self.API));           #7  (Name)
            Widget.addWidget(TableMenu(self.API));           #8  (Table Menu)
            Widget.addWidget(TableColumn(self.API));         #9  (Table Column)
            Widget.addWidget(ForeignKey(self.API));          #10 (SetUp Foreign Key)
